...
SELECT USER_NAME
  FROM USERS
 WHERE USER_YEAR = (SELECT MAX(USER_YEAR) 
                     FROM USERS);
...
SELECT U.USER_ID AS 고객아이디, U.USER_NAME AS 고객명, COUNT(*) AS 구매횟수
 ...
 HAVING COUNT(*) >= 2 -- 그룹바이 사용 이후에 쓸 수 있는 조건문 해빙절 
 ORDER BY COUNT(*);
...
-- 작성
-- 만들어져있는 상태라도 정상적인 프로시저로 바뀌게 된다. / 드랍 안하는 이유
CREATE OR REPLACE PROCEDURE USER_PROC -- 전달하는 매개변수가 없을때는 () 사용안한다.
IS
    NAME USERS.USER_NAME%TYPE; -- 변수/타입 - 테이블 동일한 칼럼타입 NAME VARCHAR2(20 BYTE) 
BEGIN -- 작업시작
    SELECT USER_NAME INTO NAME --    SELECT 칼럼 INTO 변수 
      FROM USERS
     WHERE USER_YEAR = 1960;
    DBMS_OUTPUT.PUT_LINE(NAME); -- NAME변수에 들어간 값을 출력하라
END;
...
RETURN VARCHAR2 -- 구매총액의 합계를 구해서  ABC만들기 
IS -- 변수선언
    BUY_TOTAL NUMBER;
BEGIN
    SELECT SUM(PROD_PRICE * BUY_AMOUNT) -- 구매총액 구하기
      INTO BUY_TOTAL -- 변수
      FROM BUYS -- 구매총액이 들어있는 테이블
     WHERE USER_ID = USERID; -- 조건

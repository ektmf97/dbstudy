...
/*
    트리거 실습
    사원 테이블에서 삭제할 사원정보를 퇴사자 테이블로 삽인하는 RETIRE_TRIG 생성하기 
    
    작업 순서 
    1. 사원(EMPLOYEES) 테이블의 구조를 복사하여 퇴사자(RETIRES) 테이블(RETIRE_TBL)을 생성한다.
    2. 퇴사자(RETIRES) 테이블에 퇴사번호(RETIRE_ID), 퇴사일자(RETIRE,DATE) 칼럼을 추가한다.
...
-- 2. 퇴사자(RETIRES) 테이블에 퇴사번호(RETIRE_ID), 퇴사일자(RETIRE,DATE) 칼럼을 추가한다.
ALTER TABLE RETIRES ADD RETIRE_ID NUMBER NOT NULL; -- 기본키로 쓸거라고 했으니까  NOT NULL

...
/*
    ...
    4. 칼럼 이름 : ALTER TABLE 테이블명 RENAME COLUMN 기존칼럼이명 TO 신규칼럼명 
    5. 테이블 이름 : ALTER TABLE 테이블명  RENAME TO 신규테이블명 
*/ 

-- 1. BANK-TBL 테이블에 연락처(BANK_TEL) 칼럼을 추가하시오.
...
-- 5. CUSTOMER-TBL 테이블에  GRADE 칼럼을 추가하시오.
--    GRADE 칼럼은 ('VIP  'GOLD', 'SILVER', 'BRONZE' 중 하나의 값만 가질 수 있도록 CHECK 제약조건을 지정하시오.
ALTER TABLE CUSTOMER_TBL 
    ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE IN('VIR','GOLD','SILVER','BRONZE')); 

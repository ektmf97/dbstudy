/*
    ...
    2. 드리븐(DRIVEN) 테이블
       ...
       2) 행(ROW)의 개수가 일반적으로 적고, FK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 활용이 불가능하다.
...
SELECT E.EMP_NO, E.NAME, D.DEPT_NO, D.DEPT_NAME
  FROM DEPARTMENT_TBL D INNER JOIN EMPLOYEE_TBL E -- 항상 PK를 가진 테이블 먼저 적어주기 , FK 나머지 
   ON D.DEPT_NO = E.DEPART;  -- 양쪽 테이블에 칼럼들을 사용가능
...
-- 2. 왼쪽 외부 조인 (왼쪽에 있는 테이블은 일치하는 정보가 없어도 무조건 조회한다.)
...
SELECT D.DEPT_NO, D.DEPT_NAME, E.EMP_NO, E.NAME
  FROM DEPARTMENT_TBL D, EMPLOYEE_TBL E
 WHERE D.DEPT_NO = E.DEPART(+); -- 왼쪽 조인 할거면 오른쪽 조인 조건에 (+) 해준다

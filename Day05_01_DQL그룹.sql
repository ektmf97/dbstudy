/*
    ...
    2. GROUP BY절에서 지정한 칼럼은 '반드시' SELECT절에 있어야 한다.(필수 규칙)
*/

-- 1. 동일한 DEPARTMANT_ID를 그룹화하여 조회하시오.
SELECT DEPARTMENT_ID
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID; 
 
-- 2. 동일한 DEPARTMANT_ID로 그룹화하여 FIRST_NAME과 DEPATRMENT_ID를 조회하시오. (실패하는 쿼리문)
...
SELECT
  ...
  FROM
       EMPLOYEES
 WHERE 
       DEPARTMENT_ID IS NOT NULL -- GROUP BY절을 사용하기 때문에 HAVING을 안해도 사용가능하다
 GROUP BY
       DEPARTMENT_ID;   -- 셀렉트 하려는 칼럼들은 그룹바이에 있어야한다. 집계함수는 예외
       
-- 참고사항. GROUR BY 없이 집계함수 사용하기
SELECT
...
/*
    조건 지정하기
    1. GROUP BY절로 그룺화 할 대상(모수)이 적을수록 성능에 유리하다.
    2. GROUP BY 이전에 처리할 수 있는 조건은 WHERE절로 처리하는 것이 유리하다.
    3. GROUP BY 이후에만 처리할 수 있는 조건은 HAVING절이 처리한다.
*/

-- 5. DEPARTMANT_ID가 NULL인 부서를 제외하고, 모든 부서의 부서별 사원 수를 조회하시오.
--    해설)  DEPARTMANT_ID가 NULL 부서의 제외는 GROUP BY 이전에 처리할 수 있으므로 WHERE절로 처리한다.
SELECT
       DEPARTMENT_ID
     , COUNT(*) AS 부서별사원수
  FROM

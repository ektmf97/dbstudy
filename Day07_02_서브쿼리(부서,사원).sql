...
/*
    서브쿼리의 실행 결과에 의한 구분
    1. 단일 행 서브쿼리 싱글로우서브쿼리
        1) 결과 행이 1개이다.
        2) 단일 행 서브쿼리인 대표적인 경우
           (1) WHERE절에서 사용한 동등비고(=) 칼럼이 PK, UNIQUE 칼럼인 경우
    ...
    2. 다중 행 서브쿼리
        ...
        3) 다중 행 서브쿼리에서 사용되는 연산자 
            다중 행 연산자를 사용(IN. ANY, ALL 등) - IN을 제일 많이 사용함 
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE_TBL
 WHERE POSITION = (SELECT POSITION -- = 단일행 서브쿼리여서 단일행 연산자 사용이 가능했다
                     FROM EMPLOYEE_TBL
                    WHERE EMP_NO = 1001);  -- 단일행 WHERE절에 적은  EMP_NO가 PK라서 단일행 

-- 메인쿼리가 직급을 조회하려고 준비 하면 서브쿼리도 직급을 반환 해줘야 한다.                    

-- 2. 부서번호가 2인 부서와 동일한 지역에 있는 부서를 조회하시오.
SELECT 부서정보 
  FROM 지역 = (부서번호가 2인 부서의 지역)
...
-- 다중행 DEPART PK도 아니고 유니크 칼럼도 아니기 때문에 결과가 여러 개 나올 수 있음
...
SELECT
    ...
    , (SELECT D.DEPT_NAME
        ...
        WHERE D.DEPT_NO = E.DEPART -- 동일한 데이터 대상으로 조회 
          AND D.DEPT_NO = 1)
  FROM
      EMPLOYEE_TBL E;

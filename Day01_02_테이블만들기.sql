...
/*
    ...
    데이터들의 길이가 고정되어 있을때 사용하면 좋음 모두가 같은값을 가지고 있을때
    2. VARCHAR2(size) : 가변 길이 문자 타입(size : 1 ~ 4000바이트)
    3. DATE           : 날짜/시간 타입 자바SQL 매칭하기 위한 타입
    4. TIMESTAMP      : 날짜/시간 타입(좀 더 정밀)
    5. NUMBER(p,s)    : 정밀도(p), 스케일(s)로 표현하는 숫자 타입
        1) 정밀도 : 정수부와 소수부를 모두 포함하는 전체 유효 숫자과 몇 개인가?
        2) 스케일 : 소수부의 전체 유효 숫자가 몇 개인가?
        예시)
            (1) NUMBER      : 최대 38자리 숫자(22바이트)
            (2) NUMBER(3)   : 정수부가 최대 3자리인 숫자(최대 999) 음수도 가능
            (3) NUMBER(5,2) : 최대 전체 5자리, 소수부 2자리인 숫자(123.45)
            (4) NUMVER(2,2) : 1 미만의 소수부 2자리인 실수(0.15) - 정수부의 0은 유효 자리가 아님
*/

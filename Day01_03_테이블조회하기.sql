/*
    ...
    4. 테이블 내용의 변경이 생기지 않는다.
        (트랜잭션 처리 대상이 아니고, COMMTI이 필요하지 않다.)
    5. 형식([]는 생략 가능)
        SELECT 조회할칼럼, 조회할칼럼, 조회할칼럼, ... 
        FROM 테이블이름 
        [WHERE 조건식 ]
        [GROUTP BY 그룹화할칼럼] [HAVING 그룹조건식]
        [ORDER BY 정렬할칼럼 정렬방식]
    6. 순서 
        ④SELECT 조회할칼럼, 조회할칼럼, 조회할칼럼, ... 
        ① FROM 테이블이름 
...
/*
    ...
    3. 모든 세부 작업이 성공하면 COMMIT 하고, 하나라도 실패하면 모든 세부 작업의 취소를 진행한다.
        (All or Nothing)
*/
...
-- 4) 테이블 별명 지정하기
SELECT E.ENAME
FROM EMP E; -- EMP 테이블의 별명을 E로 부여한다. (테이블 이름이 길 때 사용) AS(ALIAS)를 사용할 수 없다.
...
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO
  FROM EMP
 WHERE ENAME LIKE 'A%'; -- = 사용 불가 = 대신 LIKE 사용 

...
ALTER TABLE EMPLOYEE_TBL
    ADD CONSTRAINT FK_EMP_DEPT FOREIGN KEY(DEPART) 
        REFERENCES DEPARTMENT_TBL(DEPT_NO)
            ON DELETE SET NULL; -- 옵션이 없으면 위배가 되기 때문
...
CREATE SEQUENCE DEPARTMENT_SEQ
    ...
    NOMINVALUE     -- 번호의 하한선이 없다.(생략가능)    MINVALUE 100 : 번호의 최소값이 100이다.
...
--SELECT DEPARTMENT_SEQ.NEXTVAL FROM DUAL; -- 오라클에서는 테이블에 없는데이터를 조회하려면 DUAL 테이블을 사용한다.
...
-- 2. 지역(LOCATION)이 '서울'인 부서를 삭제하시오.('서울'에서 근무하는 사원의 부서번호가 ON DELETE SET NULL 외래키 옵션에 의해서 NULL로 처리된다.)
